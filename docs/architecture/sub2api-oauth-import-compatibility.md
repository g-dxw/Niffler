...
名称不得添加 `codex_` 前缀。外层 `name` 只有在本身是邮箱时才作为邮箱来源，不直接当作展示名。
...
- 后端从 `credentials` 中读取 `access_token`、`refresh_token`、邮箱、账号 ID、用户 ID、空间名称和套餐，并使用显式字段映射，不合并任意未知字段；当 `credentials.email` 缺失且外层 `name` 是邮箱时，回退补充邮箱。
- 每条无效记录都进入批量失败结果，错误信息不得包含 Token。
- sub2api 包装对象缺少有效的 `accounts` 数组时，批量结果明确报告格式错误。
- 缺少邮箱和用户 ID 的账号进入失败结果；如果外层 `name` 不是邮箱，空间 ID 不能单独通过身份校验。
...
- 验证记录名称分别使用“邮箱 · 空间名称”和“邮箱 · 空间 ID”，并验证外层 `name` 作为邮箱来源的兼容路径。

...
- Codex App / CLI 的流式请求里，如果用户文本是明确生图意图，且原本 `tool_choice` 是 `auto`，系统会改成 `{"type":"image_generation"}`，强制上游真正调用图片工具；该处理不改写用户选择的主模型。
...
- 影响 Codex App / CLI 的流式明确生图请求：这类请求会强制选择 `image_generation` 工具，避免客户端本地工具说明抢走生图流程。
...
- 单元测试覆盖 Codex App / CLI 流式明确生图请求会强制选择图片工具，且不改写主模型。

...
- Codex App / CLI 带 `X-Openai-Internal-Codex-Responses-Lite: true` 的请求不自动补充 `image_generation` 工具，也不移除 Lite 请求头；这类请求保留 Codex 官方客户端原本的图片展示路径。
...
- 影响 Codex App / CLI 的 Responses Lite 请求：Lite 请求保持客户端原样，不由 Niffler 自动添加图片工具或改写工具选择，避免破坏客户端图片展示。
...
- 单元测试覆盖 Codex App / CLI Responses Lite 请求不会自动补充图片工具，也不会注入图片工具说明。

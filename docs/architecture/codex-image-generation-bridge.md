...
- Codex App 带 `X-Openai-Internal-Codex-Responses-Lite: true` 的普通请求不会自动补充 `image_generation` 工具；该 Lite 上游只接受 function、custom 和客户端执行的搜索工具。
...
- 不再影响 Codex App 的 Responses Lite 普通对话请求，避免普通聊天因图片工具被上游拒绝。
...
- 单元测试覆盖 Codex App Responses Lite 普通请求不补充图片工具。

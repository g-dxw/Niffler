...
| Usage/audit | `usage`, `provider_api_key_usage_contributions`, `usage_counter_deltas`, `usage_body_objects`, `usage_body_blobs`, `usage_http_audits`, `usage_routing_snapshots`, `usage_settlement_snapshots`, `request_candidates`, `audit_logs` | `repository/usage`, `repository/candidates`, `repository/audit` | Keep core write/audit queries handwritten; provider contribution rows are rebuilt from usage and settlement facts. |

...
本文档记录已确认的产品与架构方案。当前已实现套餐按全局模型限制、请求进上游前的套餐模型判断、结算按模型扣套餐、短时额度可用性缓存，以及后台套餐编辑页的直接模型选择。提供商辅助批量选择模型仍是后续增强。
...
| 按提供商批量选择只是后续配置辅助 | 后续可以增加“选择 Codex 提供商后一键加入其当前关联全局模型”的入口；保存后仍不和提供商动态绑定。 |
...
```text
方式一：直接搜索并勾选全局模型
方式二（后续增强）：选择一个提供商，一键勾选该提供商当前关联的全局模型，再手动删减
保存结果：固定全局模型 ID 列表
```
...
| 结算字段 | 已有用户扣费和实际成本相关字段。 | 明确 `total_cost_usd` 表示用户扣费，`actual_total_cost_usd` 表示平台成本。 |

...
公共模型市场使用独立的公开支持接口，不要求用户登录，但只返回允许公开展示的数据。

| 接口 | 用途 | 主要返回内容 |
| --- | --- | --- |
| `GET /api/public/model-groups` | 获取公开用户组摘要 | 用户组 ID、名称、销售倍率、模型范围模式 |
| `GET /api/public/model-groups/catalog` | 获取分组后的公共模型目录 | 用户组、允许模型、模型能力、公开价格、使用量和聚合健康状态 |

数据关系与投影规则：

1. 只投影 `visibility = public` 的用户组。
2. 用户组的 `allowed_models_mode` 和 `allowed_models` 决定公开模型范围。
3. 如果存在与用户组 ID 对应的公开产品计划，产品计划模型列表和模型级销售倍率覆盖用户组的通用模型配置。
4. 模型健康状态由可用 Provider、Endpoint 和 Key 的聚合结果计算；同一请求内相同模型只计算一次。
5. 模型配置在返回前经过公开字段清洗，不返回上游凭据、OAuth 信息或完整管理端路由数据。
6. 当前单次读取全局模型、产品计划和计划模型时最多读取 1000 条；超过该规模时应改为分页或批量查询，不能依赖静默截断。

公开目录是展示视图，不替代真实调用鉴权。模型是否可调用仍由用户、API Key、模型权限、API Format、调度候选和实时健康状态决定。
...
站点名称、副标题和“联系我们”使用系统配置存储，不新增专用业务表。“联系我们”使用以下配置键：

| 配置键 | 说明 |
| --- | --- |
| `contact_us_enabled` | 是否在公共导航展示入口 |
| `contact_us_format` | `markdown` 或 `html` |
| `contact_us_content` | 公共展示内容 |

公开认证设置响应会携带“联系我们”的格式和内容，前端通过公共布局或对话框展示，并对 Markdown/HTML 输出进行安全清洗。“联系我们”不使用版本字段，也不参与隐私政策的版本确认流程。

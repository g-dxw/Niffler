...
管理员可在「系统设置 → 站点与注册」中维护公共站点内容。保存后，首页、公共导航和相关公开页面会读取最新配置。

| 配置 | 说明 |
| --- | --- |
| 站点名称与副标题 | 用于公共页、导航和登录相关界面 |
| “联系我们”启用状态 | 控制公共导航是否展示“联系我们”入口 |
| 内容格式 | 支持 Markdown 或 HTML |
| 内容 | 面向未登录访客和普通用户展示的联系、授权或支持信息 |

“联系我们”内容会在公开页面展示，并在前端渲染前进行安全清洗。不要在内容中填写 API Key、管理令牌、数据库密码等敏感信息。
...
未登录访客也可以访问公共站点，但不能创建 API Key 或调用需要认证的模型接口。

| 入口 | 用途 |
| --- | --- |
| `/` | 公共首页和产品能力概览 |
| `/models` | 查看公开用户组可用的模型、能力、价格倍率和健康状态 |
| `/guide` | 客户接入总览 |
| `/guide/models` | 模型选择和模型市场说明 |
| `/guide/clients` | 客户端接入说明 |
| `/guide/usage-billing` | 用量与计费说明 |
| `/guide/faq` | 常见问题 |

公共页支持简体中文和英文切换。旧版 `/guide/architecture`、`/guide/concepts`、`/guide/strategy`、`/guide/advanced`、`/guide/modules` 地址会重定向到对应的新指南入口。

公共模型市场仅展示标记为公开的用户组及其允许模型。最终能否调用仍由登录用户、API Key、模型权限、API Format 和实时调度状态共同决定；“页面可见”不等于“任意密钥均可调用”。

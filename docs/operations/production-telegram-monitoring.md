...
- 设置命令默认同时修改两台服务器。
- 命令末尾可增加 `rn01` 或 `hd0526`，只修改指定服务器，例如
...
`rn01` 不保存 `hd0526` 的 root 登录能力。它使用单独的 SSH 密钥连接
`niffler-monitor-sync` 用户；该密钥被强制绑定到监控设置脚本，只能读取监控状态
和修改上述三个整数，不能获得终端、转发端口或执行其他命令。
...
- 两台服务器各发送一条测试摘要，Telegram API 返回成功。
- `/status` 和 `/settings` 返回两台服务器的中文状态与当前设置。
- 非授权 Chat ID 无法修改设置；错误范围和预警值大于严重值会被拒绝。
- 修改单台和同时修改两台服务器均通过，修改后下一次监控读取新值。
...
- 两台服务器的监控配置和 Telegram 凭据权限均为 `0600`。

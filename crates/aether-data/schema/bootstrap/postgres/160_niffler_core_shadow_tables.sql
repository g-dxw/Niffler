...
CREATE TABLE IF NOT EXISTS public.niffler_stability_observations (
    ...
    created_at_unix_ms bigint NOT NULL,
    updated_at_unix_ms bigint NOT NULL,
    CONSTRAINT uq_niffler_stability_observations_window
        UNIQUE (window_start_unix_ms, window_end_unix_ms)
);

CREATE INDEX IF NOT EXISTS idx_niffler_stability_observations_status_time
    ON public.niffler_stability_observations (status, window_end_unix_ms);

CREATE INDEX IF NOT EXISTS idx_niffler_stability_observations_window
    ON public.niffler_stability_observations (window_end_unix_ms);

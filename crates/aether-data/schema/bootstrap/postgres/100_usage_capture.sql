...
-- External object storage metadata for complete usage bodies.
...
CREATE INDEX IF NOT EXISTS ix_provider_api_key_usage_contributions_key
    ON public.provider_api_key_usage_contributions USING btree (
        provider_api_key_id,
...
CREATE INDEX IF NOT EXISTS ix_provider_api_key_usage_contribution_backfills_ready
    ON public.provider_api_key_usage_contribution_backfills USING btree (status, updated_at);
...
CREATE INDEX IF NOT EXISTS ix_provider_api_key_usage_projection_repairs_ready
    ON public.provider_api_key_usage_projection_repairs USING btree (available_at, updated_at);

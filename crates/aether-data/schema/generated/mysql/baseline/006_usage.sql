...
CREATE TABLE IF NOT EXISTS usage_counter_deltas (
    ...
    `window_total_cost_usd_delta` DOUBLE,
...
CREATE TABLE IF NOT EXISTS provider_api_key_usage_contributions (
    ...
    `updated_at` BIGINT NOT NULL,
    PRIMARY KEY (`request_id`),
    KEY ix_provider_api_key_usage_contributions_key (`provider_api_key_id`, `usage_created_at_unix_secs`),
...
CREATE TABLE IF NOT EXISTS provider_api_key_usage_projection_repairs (
    `provider_api_key_id` VARCHAR(64) NOT NULL,
    `main_requested` TINYINT(1) NOT NULL DEFAULT 0,
    `window_requested` TINYINT(1) NOT NULL DEFAULT 0,
    `available_at` BIGINT NOT NULL,

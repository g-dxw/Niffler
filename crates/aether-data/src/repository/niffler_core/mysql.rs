...
#[derive(Debug, Clone)]
pub struct MysqlNifflerCoreRepository {
    pool: MysqlPool,
}

impl MysqlNifflerCoreRepository {
...
#[async_trait]
impl NifflerCoreReadRepository for MysqlNifflerCoreRepository {
...
#[async_trait]
impl NifflerCoreWriteRepository for MysqlNifflerCoreRepository {
...
ON DUPLICATE KEY UPDATE
  is_enabled = VALUES(is_enabled),
  sales_multiplier_override = VALUES(sales_multiplier_override),
  updated_at_unix_ms = VALUES(updated_at_unix_ms)
"#,
        )
        .bind(&record.id)
...
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
ON DUPLICATE KEY UPDATE
  enable_new_routing = VALUES(enable_new_routing),
  enable_settlement_snapshot = VALUES(enable_settlement_snapshot),
  enable_error_return_rules = VALUES(enable_error_return_rules),
  enable_billing_reservation = VALUES(enable_billing_reservation),
  enable_referral_ledger = VALUES(enable_referral_ledger),
...
ON DUPLICATE KEY UPDATE
  ...
  requested_model_name = VALUES(requested_model_name),
  upstream_execution_model_name = VALUES(upstream_execution_model_name),
  image_tool_model_name = VALUES(image_tool_model_name),
  pricing_snapshot = VALUES(pricing_snapshot),
  wallet_charge_usd = VALUES(wallet_charge_usd),
  entitlement_charge_usd = VALUES(entitlement_charge_usd),
  upstream_cost_usd = VALUES(upstream_cost_usd),
  gross_margin_usd = VALUES(gross_margin_usd),
  created_at_unix_ms = VALUES(created_at_unix_ms),
  finalized_at_unix_ms = VALUES(finalized_at_unix_ms)
"#,
        )
        .bind(&record.id)
        .bind(&record.request_id)
...
ON DUPLICATE KEY UPDATE
  ...
  product_plan_id = VALUES(product_plan_id),
  requested_model_name = VALUES(requested_model_name),
  estimated_reservation_usd = VALUES(estimated_reservation_usd),
  legacy_final_charge_usd = VALUES(legacy_final_charge_usd),
  difference_usd = VALUES(difference_usd),
  estimation_source = VALUES(estimation_source),
...
"#,
    ...
    async fn create_referral_reward_ledger(
        ...
        sqlx::query(
            r#"
INSERT IGNORE INTO niffler_referral_reward_ledger (
  id, order_id, idempotency_key, inviter_user_id, invitee_user_id, rule_id,
  reward_amount_usd, rule_snapshot, status, failure_reason, retry_count,
  paid_at_unix_ms, cancelled_at_unix_ms, created_at_unix_ms, updated_at_unix_ms
)
...
ON DUPLICATE KEY UPDATE
  ...
  upstream_status_code = VALUES(upstream_status_code),
...
ON DUPLICATE KEY UPDATE
  status = VALUES(status),
  rollback_drill_status = VALUES(rollback_drill_status),
  consistency_checked_count = VALUES(consistency_checked_count),
  consistency_issue_count = VALUES(consistency_issue_count),
  unknown_upstream_count = VALUES(unknown_upstream_count),
  legacy_write_call_count = VALUES(legacy_write_call_count),
  billing_reservation_exception_count = VALUES(billing_reservation_exception_count),
  referral_exception_count = VALUES(referral_exception_count),
...
fn build_service_count_query(query: &NifflerUpstreamServiceListQuery) -> QueryBuilder<'_, MySql> {
    let mut builder = QueryBuilder::new("SELECT COUNT(*) FROM niffler_upstream_services");
    push_service_filters(&mut builder, query);
    builder
}

fn build_service_rows_query(query: &NifflerUpstreamServiceListQuery) -> QueryBuilder<'_, MySql> {
...
fn build_account_count_query(query: &NifflerUpstreamAccountListQuery) -> QueryBuilder<'_, MySql> {
    let mut builder = QueryBuilder::new("SELECT COUNT(*) FROM niffler_upstream_accounts");
    push_account_filters(&mut builder, query);
    builder
}

fn build_account_rows_query(query: &NifflerUpstreamAccountListQuery) -> QueryBuilder<'_, MySql> {
    let mut builder = QueryBuilder::new(
...
fn build_product_plan_count_query(query: &NifflerProductPlanListQuery) -> QueryBuilder<'_, MySql> {
    let mut builder = QueryBuilder::new("SELECT COUNT(*) FROM niffler_product_plans");
    push_product_plan_filters(&mut builder, query);
    builder
}

fn build_product_plan_rows_query(query: &NifflerProductPlanListQuery) -> QueryBuilder<'_, MySql> {
    let mut builder = QueryBuilder::new(
...
fn build_consistency_check_rows_query(
    ...
    let mut builder = QueryBuilder::new(
        "SELECT ss.request_id, ss.user_id, ss.api_key_id, ss.product_plan_id, \
         ...
         CAST(u.total_cost_usd AS DOUBLE) AS usage_total_cost_usd, \
...
fn build_route_attempt_rows_query(query: &NifflerRouteAttemptListQuery) -> QueryBuilder<'_, MySql> {
    let mut builder = QueryBuilder::new(
...
fn map_service_row(row: &MySqlRow) -> Result<StoredNifflerUpstreamService, DataLayerError> {
...
fn map_account_row(row: &MySqlRow) -> Result<StoredNifflerUpstreamAccount, DataLayerError> {
    let status: String = row.try_get("status").map_sql_err()?;
...
fn map_product_plan_row(row: &MySqlRow) -> Result<StoredNifflerProductPlan, DataLayerError> {

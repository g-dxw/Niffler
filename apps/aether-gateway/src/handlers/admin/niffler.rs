...
async fn build_referral_reward_ledger_response(
    ...
    let query = NifflerReferralRewardLedgerListQuery {
        ...
        updated_at_gte_unix_ms: None,
        updated_at_lt_unix_ms: None,
        offset: parse_usize_query(request_context.query_string(), "offset").unwrap_or(0),
        limit: parse_usize_query(request_context.query_string(), "limit").unwrap_or(50),
    };
...
fn legacy_write_entrypoints() -> Vec<NifflerLegacyCodeDependency> {
    vec![
        legacy_code_dependency(
            ...
            "已迁移 Provider 返回 409，并提示去 Niffler Core / 上游服务修改；未迁移 Provider 继续旧逻辑。",
        ),
        legacy_code_dependency(
            "旧 Provider Key 写入口",
            "新增、更新、恢复、测试旧账号",
            ...
            "已迁移账号返回 409，并提示去 Niffler Core / 上游账号修改；未迁移账号继续旧逻辑。",
        ),
        legacy_code_dependency(
            "旧 Provider 端点写入口",
            "维护 Provider 上游 API 地址、协议和默认请求配置",
            Some("POST/PUT/DELETE"),
            "/api/admin/endpoints",
            "已接入冻结机制",
            "旧端点配置仍会影响 Provider 和 Provider Key 的实际请求地址。",
            "已迁移 Provider 返回 409，并提示去 Niffler Core / 上游服务修改；未迁移 Provider 继续旧逻辑。",
        ),
        legacy_code_dependency(
            "旧 Provider 模型价格写入口",
            ...
            "已迁移 Provider 返回 409，并提示去 Niffler Core / 上游服务模型价格修改；未迁移 Provider 继续旧逻辑。",
        ),
        legacy_code_dependency(
            "旧用户分组写入口",
            ...
            "已迁移分组返回 409，并提示去 Niffler Core / 产品策略修改；未迁移分组继续旧逻辑。",
        ),
        legacy_code_dependency(
            "旧用户 Key 限制写入口",
            ...
            "已迁移 Key 在旧页面修改分组时返回 409，并提示去 Niffler Core / 用户 Key 产品策略绑定修改；名称、限速、并发和功能开关继续允许。",
        ),
    ]
}

...
pub(crate) fn spawn_niffler_billing_reservation_expiry_worker(
    data: Arc<GatewayDataState>,
) -> Option<tokio::task::JoinHandle<()>> {
    ...
    Some(tokio::spawn(async move {
        if let Err(err) = run_niffler_billing_reservation_expiry_once(&data).await {
            log_maintenance_worker_failure("niffler_billing_reservation_expiry", "startup", &err);
        }
        let mut interval = tokio::time::interval(NIFFLER_BILLING_RESERVATION_EXPIRY_INTERVAL);
        interval.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Delay);
        interval.tick().await;
        loop {
            interval.tick().await;
            if let Err(err) = run_niffler_billing_reservation_expiry_once(&data).await {
                log_maintenance_worker_failure("niffler_billing_reservation_expiry", "tick", &err);
            }
        }
    }))
}

pub(crate) fn spawn_niffler_stability_observation_worker(
    data: Arc<GatewayDataState>,
) -> Option<tokio::task::JoinHandle<()>> {
    if !data.has_niffler_core_reader() || !data.has_niffler_core_writer() {
        return None;
    ...
    Some(tokio::spawn(async move {
        if let Err(err) = run_niffler_stability_observation_once(&data).await {
            log_maintenance_worker_failure("niffler_stability_observation", "startup", &err);
        }
        let mut interval = tokio::time::interval(NIFFLER_STABILITY_OBSERVATION_INTERVAL);
        interval.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Delay);
        interval.tick().await;
        loop {
            interval.tick().await;
            if let Err(err) = run_niffler_stability_observation_once(&data).await {
                log_maintenance_worker_failure("niffler_stability_observation", "tick", &err);
            }
        }
    }))
}

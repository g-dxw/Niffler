...
#[test]
fn classifies_admin_niffler_core_legacy_dependency_audit_as_admin_proxy_route() {
    let headers = headers(&[]);
    let uri: Uri = "/api/admin/niffler-core/legacy-dependency-audit?offset=0&limit=50"
        .parse()
        .expect("uri should parse");
    let decision =
        classify_control_route(&http::Method::GET, &uri, &headers).expect("route should classify");
...
#[test]
fn classifies_admin_niffler_core_consistency_checks_as_admin_proxy_route() {
    let headers = headers(&[]);
    let uri: Uri = "/api/admin/niffler-core/consistency-checks?request_id=req-1"
        .parse()
        .expect("uri should parse");
    let decision =
        classify_control_route(&http::Method::GET, &uri, &headers).expect("route should classify");
...
#[test]
fn classifies_admin_niffler_core_stability_observations_as_admin_proxy_route() {
    let headers = headers(&[]);
    let uri: Uri = "/api/admin/niffler-core/stability-observations?status=pending"
        .parse()
        .expect("uri should parse");
    let decision =
        classify_control_route(&http::Method::GET, &uri, &headers).expect("route should classify");
    ...
    assert_eq!(
        decision.route_kind.as_deref(),
        Some("list_stability_observations")
    );
    assert_eq!(
        decision.auth_endpoint_signature.as_deref(),
        Some("admin:system")
...
#[test]
fn classifies_admin_niffler_core_referral_reward_ledger_retry_as_admin_proxy_route() {
    let headers = headers(&[]);
    let uri: Uri = "/api/admin/niffler-core/referral-reward-ledger/ledger-1/retry"
        .parse()
        .expect("uri should parse");
    let decision =
        classify_control_route(&http::Method::POST, &uri, &headers).expect("route should classify");
    ...
    assert_eq!(
        decision.route_kind.as_deref(),
        Some("retry_referral_reward_ledger")
    );
    assert_eq!(
        decision.auth_endpoint_signature.as_deref(),
        Some("admin:wallets")
    ...
    assert_eq!(
        management_token_required_permission(&http::Method::POST, &decision).as_deref(),
        Some("admin:wallets:write")
    );
    assert!(!decision.is_execution_runtime_candidate());
}

#[test]
fn classifies_admin_niffler_core_referral_reward_ledger_cancel_as_admin_proxy_route() {
    let headers = headers(&[]);
    let uri: Uri = "/api/admin/niffler-core/referral-reward-ledger/ledger-1/cancel"
        .parse()
        .expect("uri should parse");
    let decision =
        classify_control_route(&http::Method::POST, &uri, &headers).expect("route should classify");
    ...
    assert_eq!(
        decision.route_kind.as_deref(),
        Some("cancel_referral_reward_ledger")
    );
    assert_eq!(
        decision.auth_endpoint_signature.as_deref(),
        Some("admin:wallets")

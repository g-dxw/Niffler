...
#[test]
fn classifies_managed_instruction_profiles_as_routing_admin_route() {
    let headers = headers(&[]);
    let uri: Uri = "/api/admin/routing/managed-instruction-profiles"
        .parse()
        .expect("uri should parse");
    let decision =
    ...
    assert_eq!(
        ...
        Some("managed_instruction_profiles")
    );
    assert_eq!(

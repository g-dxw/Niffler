...
#[tokio::test]
async fn gateway_handles_codex_client_models_manifest_without_hitting_fallback_probe() {
    let fallback_probe_hits = Arc::new(Mutex::new(0usize));
    let fallback_probe_hits_clone = Arc::clone(&fallback_probe_hits);
    let fallback_probe = Router::new().route(
        "/{*path}",
    ...
    let auth_repository = Arc::new(InMemoryAuthApiKeySnapshotRepository::seed(vec![(
        Some(hash_api_key("sk-codex-client-models")),
        unrestricted_models_snapshot("key-codex-client", "user-codex-client"),
    )]));
    let candidate_repository =
        Arc::new(InMemoryMinimalCandidateSelectionReadRepository::seed(vec![
            sample_models_candidate_row("provider-openai", "openai", "openai:chat", "gpt-5.5", 10),
        ]));
    ...
    let response = reqwest::Client::new()
        .get(format!("{gateway_url}/v1/models?client_version=0.137.0"))
        .header("authorization", "Bearer sk-codex-client-models")
        .send()
        .await
        .expect("request should succeed");
    ...
    assert_eq!(response.status(), StatusCode::OK);
    let payload: serde_json::Value = response.json().await.expect("json body should parse");
    assert!(payload["models"].as_array().is_some());
    assert!(payload.get("object").is_none());
    assert!(payload.get("data").is_none());

    let slugs: Vec<&str> = payload["models"]
        .as_array()

...
impl AppState {
    ...
    ) -> Result<StoredNifflerAccountModelCapabilityListPage, GatewayError> {
        self.data
    ...
    ) -> Result<StoredNifflerRuntimeAccountModelAccessListPage, GatewayError> {
        self.data
            .list_niffler_runtime_account_model_access(query)
            .await
            .map_err(|err| GatewayError::Internal(err.to_string()))
    }
    ...
    ) -> Result<StoredNifflerStabilityObservationListPage, GatewayError> {
        self.data
            .list_niffler_stability_observations(query)
            .await
            .map_err(|err| GatewayError::Internal(err.to_string()))
    }
    ...
    pub(crate) async fn create_niffler_upstream_service(
        ...
        let saved = self
            ...
            .await
            .map_err(|err| GatewayError::Internal(err.to_string()))?;
        self.invalidate_niffler_runtime_caches();
        Ok(saved)
    }

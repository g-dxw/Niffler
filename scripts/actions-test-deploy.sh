...
if [ "$TEST_MODE" = true ]; then
    ...
    RELEASE_ROOT="${ACTIONS_RELEASE_ROOT:?}"
    SOURCE_HEALTH_URL="${ACTIONS_SOURCE_HEALTH_URL:?}"
    PUBLIC_HEALTH_URL="${ACTIONS_PUBLIC_HEALTH_URL:?}"
    ...
    INCOMING_DIR="/opt/niffler-test-release/incoming"
    STATE_FILE="/opt/niffler-test/.niffler-deployed-commit"
    FIXED_DEPLOYER="/opt/niffler-test-release/bin/deploy-test"
    REMOTE_DIR="/opt/niffler-test"
    RELEASE_ROOT="/opt/niffler-test-release"
    SOURCE_HEALTH_URL="http://127.0.0.1:18084/_gateway/health"
    PUBLIC_HEALTH_URL="https://niffler-test.123.253.224.101.sslip.io/_gateway/health"
...
case "$command_name" in
    ...
    deploy)
        ...
        if [ ! -x "$FIXED_DEPLOYER" ]; then
            die "fixed test deployer is unavailable"
        fi
        ...
        RELEASE_ROOT="$RELEASE_ROOT" "$FIXED_DEPLOYER" \
        ...
        ;;
    *)
        die "unsupported privileged test command"
        ;;
esac
